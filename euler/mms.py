...
#
#pi      = np.pi
#p       = lambda t,x,y: 0.5*np.cos(k*x)*np.sin(k*y)
#p_x     = lambda t,x,y: -0.5*k*np.sin(k*x)*np.sin(k*y)
#p_y     = lambda t,x,y: 0.5*k*np.cos(k*x)*np.cos(k*y)
...
beta    = 1
a       = -0.025
u       = lambda t,x,y: np.log(y/beta)/kappa*np.exp(a*t)
u_t     = lambda t,x,y: a*u(t,x,y)
u_x     = lambda t,x,y: np.zeros(x.shape)
u_y     = lambda t,x,y: 1/y/kappa*np.exp(a*t)
u_xx    = lambda t,x,y: np.zeros(x.shape)
u_yy    = lambda t,x,y: -1/y/y/kappa*np.exp(a*t)

k       = 0.5*np.pi
b       = 0
v       = lambda t,x,y: np.sin(k*(y-1))*np.sin(k*x)
v_t     = lambda t,x,y: np.zeros(x.shape)
v_x     = lambda t,x,y: k*np.sin(k*(y-1))*np.cos(k*x)
v_y     = lambda t,x,y: k*np.cos(k*(y-1))*np.sin(k*x)
v_xx    = lambda t,x,y: -k*k*np.sin(k*(y-1))*np.sin(k*x) 
v_yy    = lambda t,x,y: -k*k*np.sin(k*(y-1))*np.sin(k*x)

k       = np.pi
p       = lambda t,x,y: np.sin(k*y)*np.sin(k*x)
p_x     = lambda t,x,y: k*np.sin(k*y)*np.cos(k*x)
p_y     = lambda t,x,y: k*np.cos(k*y)*np.sin(k*x)

k       = np.pi
b       = 0
v       = lambda t,x,y: np.sin(k*y)*np.sin(k*x)
v_t     = lambda t,x,y: np.zeros(x.shape)
v_x     = lambda t,x,y: k*np.sin(k*y)*np.cos(k*x)
v_y     = lambda t,x,y: k*np.cos(k*y)*np.sin(k*x)
v_xx    = lambda t,x,y: -k*k*np.sin(k*y)*np.sin(k*x) 
v_yy    = lambda t,x,y: -k*k*np.sin(k*y)*np.sin(k*x)

k       = np.pi
p       = lambda t,x,y: np.sin(k*x)
p_x     = lambda t,x,y: k*np.cos(k*x)
