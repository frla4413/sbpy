...
for k,(X,Y) in enumerate(blocks):
    initu[k] += 0.1*np.sin(2*np.pi*X)*np.cos(2*np.pi*Y)*window(X,Y,-0.25)
    initv[k] += -0.1*np.sin(2*np.pi*X)*np.cos(2*np.pi*Y)*window(X,Y,-0.25)
...
name = 'interface_plots/continuous/Nx50/sol'
