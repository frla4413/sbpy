...
def bdf2_step(op, prev_prev_state, prev_state, dt, tol):
    N = int(len(prev_state)/3)
    ...
    def F(new_state):
        ...
        return T+dt*(2/3)*S, Jt+dt*(2/3)*Js
...
def solve(grid, spatial_operator, init_u, init_v, init_p, dt, num_steps):
    ...
    tol = 1e-6
    prev_state = sol
    name_base = "plots/plot"
    filename = name_base+str(0)+'.tec'
    export_to_tecplot(grid,init_u[0],init_v[0],init_p[0],filename)
    for k in tqdm(range(num_steps)):
        if k == 0:
            ...
            prev_state = sol
            sol = bdf2_step(spatial_operator, prev_prev_state, prev_state, dt, tol)
        ...
        export_to_tecplot(grid,U[k],V[k],P[k],filename)
