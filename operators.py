...
class SBP1D:
    ...
    def __init__(self, N, dx, accuracy = 2, periodic = False):
        ...
        if accuracy == 2 and periodic == False:
        ...
        if accuracy == 2 and periodic:
            ...
            data = np.array([0.5,-0.5])
            col = np.array([0,N-2])
            BOT = sparse.bsr_matrix((data,(row,col)), shape =(1,N))
...
class SBP2D:
    ...
    def __init__(self, X, Y, accuracy_x = 2, accuracy_y = 2):
        ...
        self.sbp_xi  = SBP1D(self.Nx, 1/(self.Nx-1), accuracy_x)
        self.sbp_eta = SBP1D(self.Ny, 1/(self.Ny-1), accuracy_y)
